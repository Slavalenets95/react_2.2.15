export default class SwapiService {
  ...
  async setSessionID() {
    if (!localStorage.getItem('sessionID')) {
      ...
      //throw new Error('Похоже гостевую сессию создать не удалось.')
    }
  }
  ...
  async getGenres() {
    const ganres = await this.getResource(`genre/movie/list?api_key=${this._api_key}&language=en-US`)
    return ganres
  }
  ...
  async getPostersObj(query = 'return', page = 1) {
    //Непонятно как красиво получить totalPages

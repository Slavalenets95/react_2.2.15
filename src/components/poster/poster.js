...
export default class Poster extends React.Component {
  ...
  getGenres = (genresData) => {
    return genresData.map((item) => {
      return (
        <li key={item.id} className="poster-info__list-item">
          {item.text}
        </li>
      );
    });
  ...
  render() {
    const { title, rating, releaseDate, annotation, posterPath, id, genresData } = this.props;
    const globalRatingColor = rating <= 3 ? '#E90000' : rating <= 5 ? '#E97E00' : rating <= 7 ? '#E9D100' : '#66E900';
    const genres = this.getGenres(genresData);
    return (
      <li className="poster-list__item poster">
        ...
        <div className="poster-info poster__info">
          ...
          <ul className="poster-info__list">{genres}</ul>
